                ...
                <div class="box-body">
                    <div class="col-md-12">
                                ...
                                @foreach($langs as $lang)
                                    <div class="tab-pane @if  ($loop->first) active  @endif" id="{{$lang->display_lang}}">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" value="{{$product->translate($lang->display_lang)->title}}" name="{{$lang->display_lang}}[title]" required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="slug" class="control-label col-sm-2">@lang('commonmodule::site.slug') ({{$lang->lang}}) :</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" value="{{$product->translate($lang->display_lang)->slug}}" name="{{$lang->display_lang}}[slug]" required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="description" class="control-label col-sm-2">@lang('commonmodule::site.desc') ({{$lang->lang}}) :</label>
                                            <div class="col-sm-8">
                                                <textarea class="ckeditor" name="{{$lang->display_lang}}[description]">{!! $product->translate($lang->display_lang)->description !!}</textarea>
                                            </div>
                                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <img class="img-bordered img-thumbnail pull-left" src="{{asset('/images/products/'.$product->photo)}}" style="max-width: 100px;  max-height: 100px;" alt="">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <input class="form-control" type="number" name="sell_price" min="0" value="{{$product->sell_price}}" required>

                            </div>
                            <label class="control-label col-sm-2">@lang('commonmodule::site.price_before_discount') :</label>
                            <div class="col-sm-3">
                                <input class="form-control" type="number" name="price_before_discount" min="0" value="{{$product->price_before_discount}}" required>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <input type="number" value="{{$product->current_quantity}}" min="0" name="current_quantity" class="form-control">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                ...
                                <ul data-role="treeview-metro">
                                    @foreach($categories as $cat)
                                        <li>
                                            <input type="checkbox" data-role="checkbox" value="{{ $cat->id}}" name="product_cats[]" data-caption="{{ $cat->title}}" {{in_array($cat->id , $product->categories->pluck('id')->toArray()) ?'checked':''}}  title=""><br>
                                            @if(count($cat->child)>0)
                                                <ul>
                                                    @foreach($cat->child as $child)
                                                        <li>
                                                            <input type="checkbox" data-role="checkbox" value="{{ $child->id}}" name="product_cats[]" {{in_array($cat->id , $product->categories->pluck('id')->toArray()) ?'checked':''}}  data-caption="{{ $child->title  }}" title="">
                                                            @if($child->child->count() > 0)

                                                                <ul>
                                                                    @foreach($child->child as $pr)
                                                                        <li>
                                                                            <input type="checkbox" data-role="checkbox" value="{{ $pr->id}}" name="product_cats[]" {{in_array($pr->id , $product->categories->pluck('id')->toArray()) ?'checked':''}}  data-caption="{{ $pr->title  }}" title="">
                                                                        </li>
                                                                    @endforeach
                            ...
                            <div class="col-sm-5">
                                ...
                                <div class="col-sm-6">
                                    <select class="form-control" name="active">
                                        <option value="1" {{$product->active == 1 ?'selected':''}}>@lang('commonmodule::site.active')</option>
                                        <option value="0" {{$product->active == 0 ?'selected':''}}>@lang('commonmodule::site.unactivate')</option>
                                    </select>
                                </div>
                            </div>
